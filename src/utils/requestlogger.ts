...
export function RequestLogger(req: any, res: any, next: any) {
  ...
  form.parse(req, function (err, fields, files) {
    ...
    if (req.body) strLog += ' -- REQUEST BODY: ' + JSON.stringify(req.body);
    if (fields)
      strLog += ' -- FORM/MULTIPART FIELDS: ' + JSON.stringify(fields);
    if (files) strLog += ' -- FORM/MULTIPART FILES: ' + JSON.stringify(files);

    logger.info(
      strLog.length > 2000
        ? strLog.slice(0, 2000) +
            '--------[TRUNCATED BY LOGGER, ACTUAL BODY LENGTH: ' +
            strLog.length +
            ' chars]-------- }'
        : strLog
    );
