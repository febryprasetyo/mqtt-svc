...
async function loadConfig() {
  try {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      val = data[i].value;
      if (config_type == 'INT') val = parseInt(val);
      else if (config_type == 'FLOAT') val = parseFloat(val);
      else if (config_type == 'TEXT') val = val;
      else if (config_type == 'JSON') val = JSON.parse(val);
      else if (config_type == 'BOOLEAN') val = val.toLowerCase() == 'true';
...
function isEmpty(x: any) {
  try {
    ...
    else if (x.toString().trim() === '') return true;
    else return false;
  } catch (error) {
    return true;
  }
...
async function validateParamsAll(params: any, rules: any) {
  ...
  await validator
    .validateAll(params, rules, messageTemplates)
    .then((res) => {
      return result;
    })
    .catch((err) => {
      ...
      for (var i = 0; i < arrMsg.length; i++) {
        ...
        else failOther.push(arrMsg[i].message);
      }
      ...
      if (failRequired.length === 1) {
        ...
        result.message_id =
          'Parameters [' + failRequired + '] tidak boleh kosong';
      }
      ...
      for (var i = 0; i < failOther.length; i++) {
        ...
        result.message_id +=
          failOther[i].replace('length must be', 'panjang harus') + '; ';
      }
      throw result;
    });
}
